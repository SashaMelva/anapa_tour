...
func (s *Storage) GetAllOrganizationByCategory(id int) ([]*model.OrganizationFull, error) {
	organizations := []*model.OrganizationFull{}
	query := `select organization.id, organization.name, organization.description, organization.full_name, organization.admin_id, organization.category_id, category_organization.name  from  public.organization 
	left join category_organization On category_organization.id = organization.category_id where organization.category_id = $1`
	rows, err := s.ConnectionDB.Query(query, id) // sql.Result
